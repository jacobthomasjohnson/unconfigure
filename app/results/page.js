...
export default function ResultsPage() {
  ...
  const handleDeleteResult = async (date) => {
    ...
    const res = await fetch("/api/delete-progress", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ user_id: currentUserId, date }),
    });
  ...
  return (
    <div className="w-full max-w-md mx-auto h-full flex flex-col">
      <Header />
      <h1 className="text-xl text-left mb-2 text-neutral-200 font-bold">
        PAST GAMES
      </h1>
      <div className="mb-4">
        <div className="flex gap-2 items-center">
          ...
          <button
            ...
            disabled={isLoading}
            className={`p-2 rounded bg-blue-900 hover:bg-blue-800 text-white transition min-w-1/4 ${
      ...
      <h1 className="text-xl text-left mb-2 mt-8 text-neutral-200 font-bold">
        PLAY HISTORY
      </h1>
      {entries.length === 0 ? (
        ...
        entries.map((entry) => {
          const lastGuess =
            entry.guesses[entry.guesses.length - 1]?.guess || [];
          ...
          return (
            <div
              key={entry.date}
              className={`mb-4 border ${borderColor} p-4 rounded flex`}
            >
              <div className="grow">
